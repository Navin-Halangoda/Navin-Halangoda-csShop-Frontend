...
export default function Productcard(props){
  const product= props.product
    return(
    <Link to={"/overview/"+product.productId} ><div className="w-[300px] h-[400px] shadow-2xl m-4 relative cursor-pointer hover:[&_.button]:opacity-100 hover:[&_.primaryimage]:opacity-0">
      ...
      <div className="w-full h-[150px] bg-white absolute bottom-0 button opacity-0 transition-opacity duration-300 flex items-center justify-center">
        <button className="flex items-center justify-center w-[150px] h-[50px] border-2 border-accent2 text-accent2 hover:bg-accent2 hover:text-white transition-colors duration-200">View Details</button>
      </div>
    </div>
    </Link>
    );
