...
export default function Register() {
  ...
  async function Register() {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "user register failled"
      );
    } finally {
      setIsloading(false);
    }
    ...
    <div className="w-full h-screen bg-[url('/bg.jpg')] bg-center bg-cover bg-no-repeat flex">
      ...
      <div className="w-[50%] h-full flex justify-center items-center">
        <div className="w-[500px] h-[650px] backdrop-blur-2xl shadow-2xl rounded-3xl flex justify-center items-center flex-col p-[30px]">
          ...
          <button
            onClick={Register}
            className="w-full h-[50px] bg-org text-primmary rounded-lg border-2 border-accent font-bold text-[25px] hover:bg-transparent hover:text-secondary"
          >
            Register
          </button>

          <p className="text-primmary">
            Alredy have an account? <Link to="/loging" className="italic">Login here</Link>
          </p>
